...
public class AuthenticationUtil
{
            ...
            String userId, boolean logout, boolean setLoginCookies)
    {
        if (logout)
        {
            ...
            String currentUserId = (String) request.getSession()
        ...
        {
            String cookie = "JSESSIONID=" + request.getSession().getId() + "; Path=" + request
        ...
        if (response != null && setLoginCookies)
        {
            ...
            String loginCookie =
                    COOKIE_ALFLOGIN + "=" + Long.toString(timeInSeconds) + "; Path=" + request
                            .getContextPath() + "; Max-Age=" + TIMEOUT + ";";
            if (securedSession)
            {
            ...
            {
                String userCookie =
                        COOKIE_ALFUSER + "=" + URLEncoder.encode(userId) + "; Path=" + request
                                .getContextPath() + "; Max-Age=" + TIMEOUT + ";";
                if (securedSession)
                {
                    userCookie = userCookie + " Secure; HttpOnly;";
    ...
    {
        return (request.getSession().getAttribute(UserFactory.SESSION_ATTRIBUTE_EXTERNAL_AUTH)
                != null);
    }
